...
const ExperienceLevels = () => {
  ...
  const handleDelete = async (experienceLevelId) => {
    try {
      ...
      setSuccessMessage(`Experience Level "${experienceLevelToDelete?.name}" deleted successfully!`);
  ...
  const handleDeleteClick = (experienceLevel) => {
    setExperienceLevelToDelete(experienceLevel);
    setShowDeleteModal(true);
  };

  const handleDeleteConfirm = () => {
    if (experienceLevelToDelete) {
      handleDelete(experienceLevelToDelete.id);
    }
  };
    ...
    <AdminLayout>
      <main className="flex-1 overflow-y-auto p-6">
        ...
        <ConfirmPopup
          ...
          message={`Are you sure you want to delete "${experienceLevelToDelete?.name}"?\n\nThis action cannot be undone.`}
          type="confirm"
          confirmText="Delete"
          cancelText="Cancel"
        />
