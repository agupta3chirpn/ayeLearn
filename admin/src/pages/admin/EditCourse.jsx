...
const EditCourse = () => {
  ...
  // Fetch profile data, departments, experience levels, and course data
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const courseResponse = await axios.get(`${API_CONFIG.ENDPOINTS.COURSES}/${id}`)
        if (courseResponse.data.success) {
          ...
          setCourseDetails({
            ...
            assessmentCriteria: parseJsonField(course.assessment_criteria),
            keySkills: parseJsonField(course.key_skills)
          })
          ...
          if (course.modules && course.modules.length > 0) {
            ...
            setModules(course.modules.map(module => ({
              ...
              videoHeading: module.video_heading || '',
              videos: Array.isArray(module.videos) ? module.videos : [],
              assessmentName: module.assessment_name || '',
              assessmentLink: module.assessment_link || '',
              uploadedFiles: {
                documents: Array.isArray(module.documents) ? module.documents.map(doc => ({
                  ...doc,
                ...
                videos: Array.isArray(module.videos) ? module.videos.map(video => ({
                  ...
                  originalName: video.original_name || video.originalName || '',
                  filePath: video.file_path || video.filePath || '',
                  fileType: video.file_type || video.fileType || '',
          ...
          // Set practice files if they exist
          if (course.practice_files && Array.isArray(course.practice_files) && course.practice_files.length > 0) {
            console.log('Setting practice files:', course.practice_files)
            console.log('Practice files length:', course.practice_files.length)
            const mappedPracticeFiles = course.practice_files.map(file => ({
              ...file,
            ...
            setUploadedFiles(prev => ({
              ...prev,
              practiceFiles: mappedPracticeFiles
            }))
            setFileUploads(prev => {
              console.log('Setting fileUploads.practiceFiles:', mappedPracticeFiles)
              return {
                ...prev,
                practiceFiles: mappedPracticeFiles
              }
          ...
          // Set assigned learners if they exist
          if (course.assigned_learners && Array.isArray(course.assigned_learners) && course.assigned_learners.length > 0) {
            console.log('Setting assigned learners:', course.assigned_learners)
            setAssignedLearners(course.assigned_learners.map(learner => learner.id || learner.learner_id).filter(Boolean))
          }
          setCourseDataLoaded(true)
          } catch (courseError) {
            console.error('Error processing course data:', courseError)
            // Set default values if course data processing fails
            setCourseDetails({
              title: '',
              department: '',
              level: '',
  ...
  // Update assigned learners when both course data and available learners are loaded
  useEffect(() => {
    if (availableLearners.length > 0 && !isLoadingData) {
      // This will be handled by the course data loading
      console.log('Available learners loaded, assigned learners:', assignedLearners)
    }
  }, [availableLearners, isLoadingData, assignedLearners])

  // Calculate total file size from existing files when course data is loaded
  useEffect(() => {
    if (courseDataLoaded) {
      let totalSize = 0
      ...
      // Add practice files
      uploadedFiles.practiceFiles.forEach(file => {
        totalSize += file.size || 0
      })
      
      setTotalFileSize(totalSize)
    }
  }, [courseDataLoaded, modules, uploadedFiles.practiceFiles])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const courseResponse = await axios.put(`${API_CONFIG.ENDPOINTS.COURSES}/${id}`, courseData)

      if (!courseResponse.data.success) {
        throw new Error(courseResponse.data.message || 'Failed to update course')
      }
      ...
      // Show success popup and redirect after 5 seconds
  ...
  const getFileUrl = (filePath) => {
    if (!filePath) return '#'
    const url = `${API_CONFIG.BASE_URL}${filePath}`
    console.log('Generated file URL:', url, 'from path:', filePath)
    return url
  }

  // Helper function to delete file from server
  const deleteFileFromServer = async (filePath, fileId) => {
    try {
      const response = await axios.delete(`${API_CONFIG.ENDPOINTS.COURSES}/delete-file`, {
  ...
  return (
    <AdminLayout>
                                        ...
                                        <div className="flex items-center space-x-3">
                                          ...
                                          <div className="flex-1">
                                            <div className="flex items-center space-x-2">
                                              <span className="text-sm text-gray-700 font-medium">{file.originalName || file.name}</span>
                                            </div>
                                        ...
                                        <button
                                          onClick={async () => {
                                            if (file.filePath && file.id) {
                                              const success = await deleteFileFromServer(file.filePath, file.id)
                                              if (success) {
                                                removeModuleFile(module.id, 'documents', index)
                                              } else {
                                          ...
                                          title="Delete file from server and database"
                                        >
                                          <X className="w-4 h-4" />
                                        </button>
                                      </div>
                                        ...
                                        <button
                                          onClick={async () => {
                                            if (file.filePath && file.id) {
                                              const success = await deleteFileFromServer(file.filePath, file.id)
                                              if (success) {
                                                removeModuleFile(module.id, 'videos', index)
                                              } else {
                                                alert('Failed to delete file from server')
                                              }
                                            } else {
                      ...
                      <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="text-right">
                          <p className="text-sm text-gray-600">
                            Total Size: {formatFileSize(totalFileSize)} / {(MAX_TOTAL_SIZE / (1024 * 1024)).toFixed(0)}MB
                          </p>
                          <p className="text-xs text-gray-500">
                                  ...
                                  <button
                                    onClick={async () => {
                                      if (file.filePath && file.id) {
                                        const success = await deleteFileFromServer(file.filePath, file.id)
                                        if (success) {
                                          removeFile('practiceFiles', index)
                                        } else {
                                          alert('Failed to delete file from server')
                                        }
                                      } else {
      ...
      <ConfirmPopup
        ...
        message={`The course "${courseDetails.title}" has been successfully updated. You will be redirected to the courses list in 5 seconds.`}
        type="success"
        autoClose={true}
        redirectAfter={() => navigate('/courses')}
      />
    </AdminLayout>
  )
}
