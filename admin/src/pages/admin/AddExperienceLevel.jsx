...
const AddExperienceLevel = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(API_CONFIG.ENDPOINTS.EXPERIENCE_LEVELS, {
        ...formData,
      ...
      if (error.response?.data?.errors) {
        ...
        setSubmitError(error.response?.data?.message || 'Failed to create experience level');
      }
    } finally {
      setLoading(false);
    }
    ...
    <AdminLayout>
      <main className="flex-1 overflow-y-auto p-6">
               ...
               <div className="max-w-full mx-auto">
                 {/* Header */}
                 <div className="mb-8">
                   <h1 className="text-3xl font-bold text-gray-900">Add Experience Level</h1>
                   <p className="text-gray-600 mt-2">Create a new experience level for learners</p>
                 </div>
        ...
        <ConfirmPopup
          ...
          type="success"
          autoClose={true}
          redirectAfter={() => navigate('/experience-levels')}
        />
      </main>
    </AdminLayout>
  );
