...
const EditCourse = () => {
      ...
      try {
        ...
        if (courseResponse.data.success) {
          ...
          setCourseDetails({
            ...
            assessmentCriteria: course.assessment_criteria ? JSON.parse(course.assessment_criteria) : [''],
            keySkills: course.key_skills ? JSON.parse(course.key_skills) : ['']
          })
            ...
            setModules(course.modules.map(module => ({
              ...
              uploadedFiles: {
                documents: module.documents.map(doc => ({
                  ...
                  size: doc.file_size || 0 // Add default size
                })) || [],
                videos: module.videos.map(video => ({
                  ...
                  size: video.file_size || 0 // Add default size
                })) || []
              }
            })))
          }
          ...
          if (course.practice_files && course.practice_files.length > 0) {
            ...
            const mappedPracticeFiles = course.practice_files.map(file => ({
              ...
              size: file.file_size || 0 // Add default size
            }))
          ...
          if (course.assigned_learners && course.assigned_learners.length > 0) {
            console.log('Setting assigned learners:', course.assigned_learners)
            setAssignedLearners(course.assigned_learners.map(learner => learner.id))
          }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Navigate to courses list after successful update
      navigate('/courses')
    } catch (error) {
